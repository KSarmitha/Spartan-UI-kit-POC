...
import { XvuiButtonDirective } from './hlm-button.directive';

@Component({
  selector: 'xvui-loadable-component',
  standalone: true,
  imports: [CommonModule, XvuiButtonDirective],
  template: `
    @if (loading) {
      <button disabled xvuiBtn [variant]="'destructive'" class="mr-4">
    ...
    @else {
      <button xvuiBtn class="mr-4" [variant]="'destructive'">
